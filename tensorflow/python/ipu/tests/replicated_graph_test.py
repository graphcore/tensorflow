...
class TestReplicatedGraph(test_util.TensorFlowTestCase):
  ...
  def testCreateSimpleReplicatedInfeedOutfeedWithPrefetch(self):
    ...
    with sl.Session() as sess:
      ...
      # We prefetch a full batch so each replica is processing the same data at any given iteration.
      # We are doing the folowing operation

      # We have one (v+x[i]) summed across the replicas so our operation is:
      # for i in range(0, 5):
      #   v = (v + x[i]) + (v + x[i])
      ...
      # v = 16 + 1 + 16 + 1
      # v = 34
      self.assertAllClose(result[0], np.broadcast_to(34, shape))
      outfed_result = sess.run(outfed)

      self.assertTrue(outfed_result.shape[0], 2)
      self.assertAllClose(outfed_result[0][0], outfed_result[0][1])
      self.assertAllClose(outfed_result[0][0], np.broadcast_to(0, shape))

      self.assertAllClose(outfed_result[1][0], outfed_result[1][1])
      self.assertAllClose(outfed_result[1][0], np.broadcast_to(2, shape))

      self.assertAllClose(outfed_result[2][0], outfed_result[2][1])
      self.assertAllClose(outfed_result[2][0], np.broadcast_to(8, shape))

      self.assertAllClose(outfed_result[3][0], outfed_result[3][1])
      self.assertAllClose(outfed_result[3][0], np.broadcast_to(16, shape))
