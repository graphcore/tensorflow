...
class DropoutTest(test_util.TensorFlowTestCase):
    ...
    def stage5(x, s1, s2, s3, s4):
      with variable_scope.variable_scope("vs", use_resource=True):
        weight = variable_scope.get_variable(
            ...
            initializer=init_ops.ones_initializer())
      x = math_ops.matmul(x, weight)
      _, s5 = _dropout(x, noise_shape=[32, 1])
  ...
  def testMultipleExecutionsNoUserSeed(self):
    def model(x):
      return gen_poprand_ops.ipu_dropout(x, rate=0.5, scale=1.0)
  ...
  def testFwdBwdSeedMatches(self):
    def model():
      ...
      loss, s = gen_poprand_ops.ipu_dropout(w, rate=0.5, scale=1.0)
      g = gradients_impl.gradients(loss, [w])
      assert len(g) == 1
      dropout_grad = g[0]
      assert dropout_grad.op.type == "IpuDropoutWithSeed"
