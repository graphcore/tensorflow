...
class IPUConfigTest(test_util.TensorFlowTestCase):
  ...
  def testProfilingProfiling(self):
    cfg = ipu.config.IPUConfig()
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.enable_ipu_trace_events, False)
    cfg._profiling.profiling = True
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.enable_ipu_trace_events, True)

  def testProfilingEnableIPUEvents(self):
    cfg = ipu.config.IPUConfig()
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.enable_ipu_trace_events, False)
    cfg._profiling.enable_ipu_events = True
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.enable_ipu_trace_events, True)

  def testProfilingAndEnableIPUEvents(self):
    # Check we can't use profiling and enable_ipu_events together
    cfg = ipu.config.IPUConfig()
    cfg._profiling.profiling = True
    cfg._profiling.enable_ipu_events = True
  ...
  def testProfilingUsePoplarTextReport(self):
    cfg = ipu.config.IPUConfig()
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.enable_poplar_reports_text, False)
    cfg._profiling.use_poplar_text_report = True
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.enable_poplar_reports_text, True)

  def testProfilingUsePoplarCBORReport(self):
    cfg = ipu.config.IPUConfig()
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.enable_poplar_reports_cbor, False)
    cfg._profiling.use_poplar_cbor_report = True
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.enable_poplar_reports_cbor, True)

  def testProfilingProfileExecutionEnum(self):
    cfg = ipu.config.IPUConfig()
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.execution_trace_type,
                     IpuExecutionProfileType.NO_PROFILE)
    cfg._profiling.profiling = True
    cfg._profiling.profile_execution = \
        ipu.utils.ExecutionProfileType.IPU_PROFILE
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.execution_trace_type,
                     IpuExecutionProfileType.IPU_PROFILE)

  def testProfilingProfileExecutionBool(self):
    cfg = ipu.config.IPUConfig()
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.execution_trace_type,
                     IpuExecutionProfileType.NO_PROFILE)
    cfg._profiling.profiling = True
    cfg._profiling.profile_execution = True
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.execution_trace_type,
                     IpuExecutionProfileType.DEVICE_PROFILE)

  def testProfilingEnabledToUseProfileExecution(self):
    # Check we must set profiling to True to use profile_execution
    cfg = ipu.config.IPUConfig()
    self.assertFalse(cfg._profiling.profiling)
    cfg._profiling.profile_execution = True
    with self.assertRaisesRegex(Exception,
                                "_profiling.profiling is required when"):
      cfg._create_protobuf()

  def testProfilingEnablePoplarSerializedGraph(self):
    cfg = ipu.config.IPUConfig()
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.enable_poplar_graph, False)
    cfg._profiling.enable_poplar_serialized_graph = True
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.enable_poplar_graph, True)

  def testProfilingReportEveryNthExecution(self):
    cfg = ipu.config.IPUConfig()
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.report_every_nth_execution, 0)
    cfg._profiling.report_every_nth_execution = 100
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.report_every_nth_execution, 100)

  def testProfilingMaxReportSize(self):
    cfg = ipu.config.IPUConfig()
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.max_report_size, 0x10000000)
    cfg._profiling.max_report_size = 1024768
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.max_report_size, 1024768)

  def testProfilingReportDirectory(self):
    cfg = ipu.config.IPUConfig()
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.report_directory, "")
    cfg._profiling.report_directory = "/a/test/path"
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.report_directory, "/a/test/path")

  def testProfilingGraphPoplarOptions(self):
    cfg = ipu.config.IPUConfig()
    pb = cfg._create_protobuf()
    self.assertEqual(len(pb.profiling.graph_options), 0)
    cfg._profiling.graph_poplar_options = {'A': 'B', 'C': 'D'}
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.graph_options[0].option, 'A')
    self.assertEqual(pb.profiling.graph_options[0].value, 'B')
    self.assertEqual(pb.profiling.graph_options[1].option, 'C')
    self.assertEqual(pb.profiling.graph_options[1].value, 'D')

  def testProfilingExecutionPoplarOptions(self):
    cfg = ipu.config.IPUConfig()
    pb = cfg._create_protobuf()
    self.assertEqual(len(pb.profiling.execution_options), 0)
    cfg._profiling.execution_poplar_options = {'A': 'B', 'C': 'D'}
    pb = cfg._create_protobuf()
    self.assertEqual(pb.profiling.execution_options[0].option, 'A')
    self.assertEqual(pb.profiling.execution_options[0].value, 'B')
    self.assertEqual(pb.profiling.execution_options[1].option, 'C')
    self.assertEqual(pb.profiling.execution_options[1].value, 'D')
