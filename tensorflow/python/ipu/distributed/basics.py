...
class HorovodBasics:
  ...
  def __init__(self, library_path):
    logging.info("Loading Horovod library: {}".format(library_path))
    self.MPI_LIB_CTYPES = ctypes.CDLL(library_path, mode=ctypes.RTLD_GLOBAL)

    if not self.mpi_built():
      raise RuntimeError("Internal error: The loaded Horovod library {} was "
                         "not built with MPI support.".format(library_path))
  ...
  def local_rank(self):
    """A function that returns the local Horovod rank of the calling process,
    within the node that it is running on. For example, if there are seven
    processes running on a node, their local ranks will be zero through six,
    inclusive.
  ...
  def is_homogeneous(self):
    ...
    Returns:
      A boolean value indicating whether every node in the cluster has same
      number of ranks.
    """
  ...
  def mpi_threads_supported(self):
    """A function that returns a flag indicating whether MPI multi-threading is
    supported.

    If MPI multi-threading is supported, users may mix and match Horovod usage
    with other MPI libraries, such as `mpi4py`.
    ...
    if not bool(mpi_enabled):
      raise ValueError(
          'MPI is not enabled; Please make sure it\'s installed and enabled.')
  ...
  def mpi_enabled(self):
    ...
    If MPI is enabled, users can use it for controller or data transfer
    operations.
  ...
  def gloo_enabled(self):
    ...
    If Gloo is enabled, users can use it for controller or data transfer
    operations.
