...
def _compile_function(func,
  ...
  # Add the external resource captures to arguments, and remove placeholders
  # for constants and bake them in to the graph.
  for tensor in func_graph.external_captures:
    if not allow_external_captures and tensor.dtype != dtypes.resource:
      if isinstance(tensor, ops.EagerTensor):
        # Find out the constant value for this constant, and add it to the
        # list of captured arguments, but discard the placeholder.
        constant_value = tensor_util.constant_value(tensor)
        if constant_value is not None:
          func_graph.pop_capture(tensor)
          captured_args.append(constant_value)
          continue

      if hasattr(tensor, "name"):
        name = tensor.name
      else:
        name = ops.tensor_id(tensor)
      raise _InvalidCaptureException(name)
    captured_args.append(tensor)
