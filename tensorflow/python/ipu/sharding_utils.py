...
def _sharded_backprop_gradient_function(op_name, attr_tuple, *args, **kwargs):
  ...
  if _sharding_supported():
    assert len(attr_tuple) >= 2, "Attributes should contain sharding info."

    attr_name, value = attr_tuple[-2:]
    if attr_name == sharding._XLA_SHARDING:
...
def _record_gradient_with_sharding(op_type, inputs, attrs, *args, **kwargs):
  if _sharding_supported() and op_type != "PipelineStage":
    graph = ops.get_default_graph()

    # pylint: disable=protected-access
    scoped_attrs = graph._attr_scope_map
    shard = scoped_attrs.get(sharding._XLA_SHARDING)
    if shard:
      shard_attrs = (sharding._XLA_SHARDING, shard)
      attrs = attrs + shard_attrs if attrs is not None else shard_attrs

  return backprop._record_gradient(op_type, inputs, attrs, *args, **kwargs)  # pylint: disable=protected-access
