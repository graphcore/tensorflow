...
// Helper to generate XLA hints by traversing the call tree from an initial
// function. A call stack is maintained by entering and exiting nodes separately
// so that transitive effects can be applied to the entire call stack.
void GenerateXlaHintsHelper(const string& initial_func_name,
...
absl::flat_hash_map<string, GrapplerXlaHints> GenerateXlaHints(
  ...
  const auto generate_hints_for_xla_launches = [&](const protobuf::RepeatedPtrField<NodeDef>& node_defs) {
    NameAttrList fn_name;
    for (const NodeDef& node : node_defs) {
      if (IsXlaLaunch(node) && GetNodeAttr(node, "function", &fn_name).ok()) {
