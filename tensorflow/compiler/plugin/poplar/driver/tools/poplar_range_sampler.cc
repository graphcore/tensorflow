...
Status RangeSampler::Sample(poplar::Graph& graph, poplar::Tensor& samples,
  ...
  seq.add(poplar::program::Copy(out, samples));
  return Status::OK();
}
...
StatusOr<poplar::Tensor> RangeSampler::Expectation(
    ...
    poplar::program::Sequence& seq) {
  const std::string fn_name = debug_name_ + "/Expectation";
  // E[X] = k*P[X=x]
  ...
  popops::mapInPlace(graph, expect_fn, {probabilities}, seq, fn_name);
...
StatusOr<poplar::Tensor> RangeSampler::Probabilities(
    ...
    poplar::program::Sequence& seq) {
  std::string fn_name = debug_name_ + "/Probabilities";
  switch (distribution_) {
    case DistributionType::UNIFORM: {
      // X~U[0, N-1], P[X=x] = 1/N
      poplar::Tensor probabilities =
          graph.clone(poplar::FLOAT, samples, fn_name);
      float prob = 1.0f / static_cast<float>(range_max_);
      popops::fill(graph, probabilities, seq, prob, fn_name);
      return probabilities;
    }
    case DistributionType::LOG_UNIFORM: {
      ...
      return popops::map(graph, log_uniform_pdf, {samples}, seq, fn_name);
    }
    default:
      return xla::Unimplemented("Distribution not supported.");
  }
...
Status UniqueRangeSampler::Sample(poplar::Graph& graph, poplar::Tensor& samples,
                                  poplar::program::Sequence& seq) {
  const std::string fn_name = DebugName() + "/Sample";
  auto cs = graph.addComputeSet(fn_name + "/SequentialSample");
  ...
  popops::fill(graph, bitmasks, seq, 0u, fn_name);

  // Create a num_tries tensor to calculate expectation later
  num_tries_ = graph.addVariable(poplar::INT, {}, fn_name + "/num_tries");
  ...
  poprand::setSeed(graph, Seed(), 0u, seq, fn_name);
  seq.add(poplar::program::Execute(cs));
  poplar::setHwSeeds(graph, oldSeed, seq, fn_name);
  return Status::OK();
}
...
StatusOr<poplar::Tensor> UniqueRangeSampler::Expectation(
    poplar::Graph& graph, const poplar::Tensor& samples, const uint64 k,
    poplar::program::Sequence& seq) {
  const std::string fn_name = DebugName() + "/Expectation";
...
StatusOr<std::unique_ptr<RangeSampler>> RangeSamplerFactory(
  ...
  if (unique) {
    sampler = absl::make_unique<UniqueRangeSampler>(dist_type, debug_name,
                                                    range_max, tile, seed);
  } else {
    sampler = absl::make_unique<RangeSampler>(dist_type, debug_name, range_max,
                                              tile, seed);
  }
