...
HloPoplarUseDescriptions HloGroupNormGradInstruction::GetUseDescriptions()
    const {
  return UseDescriptionsNoInputOutputAlias();
}

HloPoplarBufferDescriptions HloGroupNormGradInstruction::GetBufferDescriptions()
    const {
  return BufferDescriptionsAllocatesAllOutputs(this);
}

const FindConsumersExtensionResults HloGroupNormGradInstruction::FindConsumers(
    FindConsumersExtensionParams params) const {
  return FindConsumersExtensionResults::DoNotFindConsumers();
}

bool HloGroupNormGradInstruction::AllowNonInplaceLowering() const {
  return false;
}
...
static HloPoplarInstructionFactory group_norm_grad_factory(
    PoplarOp::GroupNormGrad, HloGroupNormGradFactoryFunc);
}  // namespace
