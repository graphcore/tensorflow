...
struct TensorOrRemoteBuffer {
   ...
   */
  explicit TensorOrRemoteBuffer(const poplar::Tensor& tensor)
      : element(tensor) {}

  /**
   * Construct with a poplar remote buffer.
   */
  explicit TensorOrRemoteBuffer(const poplar::RemoteBuffer& rbuffer)
      : element(rbuffer) {}
   ...
   * unambiguous.
   */
  poplar::Tensor& AsTensor() { return *this; }
  const poplar::Tensor& AsTensor() const { return *this; }

  /**
   * Helper functions to force the cast to a poplar remote buffer when it is
   * unambiguous.
   */
  poplar::RemoteBuffer& AsRemoteBuffer() { return *this; }
  const poplar::RemoteBuffer& AsRemoteBuffer() const { return *this; }

  /**
   * Operator overloads to support implicit casts.
   */
  operator poplar::Tensor&() { return absl::get<0>(element); }
  operator const poplar::Tensor&() const { return absl::get<0>(element); }
  operator poplar::RemoteBuffer&() { return absl::get<1>(element); }
  operator const poplar::RemoteBuffer&() const { return absl::get<1>(element); }

  /**
   * Support assignment, like this is a poplar tensor.
   */
  TensorOrRemoteBuffer& operator=(const poplar::Tensor& tensor) {
    element = tensor;
    return *this;
  }
   ...
   */
  TensorOrRemoteBuffer& operator=(const poplar::RemoteBuffer& rbuffer) {
    element = rbuffer;
    return *this;
  }
