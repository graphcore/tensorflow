...
StatusOr<bool> ParsePoplarBackendConfig::Run(HloModule* module) {
  ...
  for (auto* comp : module->computations()) {
    for (auto instr : comp->instructions()) {
      ...
      poplar_config.set_stochastic_rounding(PoplarBackendConfig::NOT_SET);
      ...
      if (stochastic_rounding_attribute != attributes.map().end()) {
        if (stochastic_rounding_attribute->second == "on") {
          poplar_config.set_stochastic_rounding(PoplarBackendConfig::FORCE_ON);
        } else if (stochastic_rounding_attribute->second == "off") {
          poplar_config.set_stochastic_rounding(PoplarBackendConfig::FORCE_OFF);
        } else {
          return xla::InvalidArgument(
              "Invalid StochasticRounding value in FrontendAttributes");
        }
      }
