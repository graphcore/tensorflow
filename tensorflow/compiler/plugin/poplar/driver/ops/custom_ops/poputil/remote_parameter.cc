...
class RemoteParameterLoadOp : public PoplarOpDef {
  StatusOr<poplar::program::Program> Creator(poplar::Graph& graph,
    ...
    if (!UseSyntheticData()) {
      ...
      poplar::RemoteBuffer remote_buffer = inputs[0];
...
class RemoteParameterStoreOp : public PoplarOpDef {
  StatusOr<poplar::program::Program> Creator(poplar::Graph& graph,
    ...
    poplar::RemoteBuffer remote_buffer = inputs[0][0];

    if (!UseSyntheticData()) {
      TensorOrRemoteBufferVector update =
          FindInstructionInputs(tensor_map, res, inst, 1, seq);

      CHECK_EQ(update.size(), 1);
      poplar::Tensor tensor = update[0];
