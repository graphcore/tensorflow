...
Status DeferredAllocations::AddDeferredAllocation(
  ...
  if (allocate_now) {
    ...
    TF_ASSIGN_OR_RETURN(DriverTensor tensor, allocate_fn(location));
  ...
  VLOG(3) << "Deferring allocation of " << location.instruction->name()
...
Status DeferredAllocations::AllocateIfExists(const HloInstruction* inst,
  ...
  while (itr != location_lookup_table_.upper_bound(end_location)) {
    VLOG(3) << "Forced allocation for input location ("
...
Status DeferredAllocations::MakeAllocation(TensorLocation input_location,
                                           TensorLocation allocation_location) {
  VLOG(3) << "Allocating for input location ("
...
Status DeferredAllocations::PostProcessAllocation(
    TensorLocation allocation_location, DriverTensor tensor) {
  TensorLocation input_location =
  ...
  for (const TensorLocation& location : flat_locations) {
    VLOG(3) << "Setting the allocation at (" << location.instruction->name()
            << "," << location.flattened_output_tuple_index << ").";
    TF_RETURN_IF_ERROR(AddOutputTensor(tensor_map_, location.instruction,
                                       location.flattened_output_tuple_index,
                                       tensor));
...
Status DeferredVisitor::HandleParameterTensor(TensorLocation input_location,
  ...
  if (!allocated[input_location.flattened_output_tuple_index] &&
    ...
    DriverTensor output = *callsite_tensor;
  ...
  if (callsite_tensor && !reallocate_input) {
    ...
    auto& graph =
        GetGraphWithOutputIndex(resources_, input_location.instruction,
                                input_location.flattened_output_tuple_index);
    DriverTensor output = TensorCloneAndRebalanceAliasing(
  ...
  for (int64_t operand_idx = 0; operand_idx != inst->operand_count();
    ...
    for (size_t i = 0; i < shapes.size(); i++, output_tuple_index++) {
      ...
      if (inputs[operand_idx][i] && inputs[operand_idx][i]->IsTensor()) {
        ...
        DriverTensor output = *inputs[operand_idx][i];
        TF_RETURN_IF_ERROR(
        ...
        VLOG(3) << "Deferring use of " << inst->name() << " operand "
...
Status DeferredVisitor::HandleCopy(HloInstruction* inst) {
  ...
  int64_t tuple_idx = 0;
  auto leaves = clone_method_tree.leaves();
  for (auto it = leaves.begin(); it != leaves.end(); ++it, ++tuple_idx) {
    const auto& leaf = *it;
    ...
    // TODO(T54942): remove it.
    bool allocate_now =
        clone_method != CloneMethod_PreserveOrderAndAliases &&
        clone_method != CloneMethod_PreserveOrderUnlessAliases &&
        clone_method != CloneMethod_Bypass &&
    ...
    if (!input && can_defer && !input_subshape.IsOpaque() &&
      ...
      // desired clone method. Don't do actual copy yet (allocation function
      // will be called for the most layout sensitive instruction and this is
      // just a fallback).
      DeferredAllocateFunction allocate_fn =
    ...
    if (input->IsTensor()) {
      ...
      if (allocate_now) {
        ...
        TF_ASSIGN_OR_RETURN(
            out, AddTensor(graph, output_location, input_subshape, resources_,
                           tensor_map, {dnai}));
        ...
        switch (clone_method) {
          ...
          default:
            return xla::FailedPrecondition(
                "Found invalid clone method for a copy instruction '%s' at "
                "input "
                "%d.",
                inst->name(), tuple_idx);
        }
...
Status DeferredVisitor::HandleDeferredWideConst(HloInstruction* inst) {
  ...
  TF_ASSIGN_OR_RETURN(DriverTensor broadcasted_tensor,
...
Status DeferredVisitor::FinishScopedVisit(HloInstruction* inst) {
  ...
  for (auto input_location : deferred_allocation->GetNotAllocatedLocations()) {
    ...
    if (allocate_all_input_tensors_ || is_required_parameter) {
      VLOG(3) << "Allocating input " << inst->ToString() << " index "
              << input_location.flattened_output_tuple_index;
      TF_RETURN_IF_ERROR(deferred_allocation->MakeDeferredAllocation(
...
Status DeferredVisitor::PropagateDeferredAllocationsOperand(
  ...
  for (size_t i = 0; i < shapes.size(); i++) {
    if (!callsite_input[i]) {
      VLOG(3) << "Propagating allocated tensor at callsite "
              << callsite_inst->name() << " for input (" << input_inst->name()
              << ", " << i << ").";
      DriverTensor t = computation_inputs_[parameter_idx][i];
...
StatusOr<DriverTensor> DeferredVisitor::AllocateInput(
    TensorLocation allocation_location, const Shape& shape,
    const poplar::DebugNameAndId& debug_name_and_id) {
  return AllocateInput(allocation_location, shape,
                       absl::optional<DriverTensor>(absl::nullopt),
                       debug_name_and_id);
}
  ...
  for (int64_t i = 0; i < num_tensors; i++) {
    switch (alias_type[i]) {
      ...
      case AliasType::PARTIAL_ALIAS: {
        ...
        VLOG(3) << "Adding a partial copy in "
                << debug_name_and_id.getPathName() << " for tuple index " << i;
        unaliased_loop_outputs[i] =
            graph.clone(loop_outputs[i].AsTensor(),
                        {debug_name_and_id,
  ...
  for (int64_t i = 0; i < num_tensors; i++) {
    switch (alias_type[i]) {
      ...
      case AliasType::NO_ALIAS_USED: {
        ...
        VLOG(3) << "Adding a output to input copy in "
                << debug_name_and_id.getPathName() << " for tuple index " << i;
