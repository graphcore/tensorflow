...
bool PoplarShapeMatchesXLAShape(const poplar::RemoteBuffer& remote_buffer,
                                const xla::Shape& shape) {
  unsigned element_count = 1;
  for (size_t d = 0; d < shape.rank(); d++) {
    element_count *= (unsigned)shape.dimensions(d);
  }
...
Status AddOutputRemoteBuffer(TensorMap& map, const HloInstruction* inst,
                             int64 n, const poplar::RemoteBuffer& rbuffer) {
