...
xla::StatusOr<bool> CustomExtractOutsideCompilation(
  ...
  for (const auto& iter : clusters) {
    ...
    if (has_outside_compilation && repeat_count > 1) {
      // We must call the extracted host function repeatedly the same number of
      // times as the compiled loop for the communication to match up, so wrap
      // it in a while loop.
      ...
      TF_RETURN_IF_ERROR(RewriteAsLoopBodyFunction(fld, host_graph_func_name));

      // Add the while loop calling the function from the outer host-side graph.
      ...
      // and remove the function. In the case of !has_outside_compilation, it
      // will just expand and remove an empty function.

      const string pivot_name = absl::StrCat(xla_cluster_name, "/pivot");
