...
std::string GetPipelineTemplateHloString() {
  return R"(
    ...
    %WeightUpdate (arg0: $DT[$R,$E], arg1: $DT[$R,$E], arg2: $AT[$R,$E]) -> $DT[$R,$E] {
    %arg0 = $DT[$R,$E] parameter(0), metadata={op_name="XLA_Args/a"}, backend_config="{}"
    %arg1 = $DT[$R,$E] parameter(1), metadata={op_name="XLA_Args/b"}, backend_config="{}"
    %arg2 = $AT[$R,$E] parameter(2), metadata={op_name="XLA_Args/c"}, backend_config="{}"
    %arg2_convert = $DT[$R,$E] convert(%arg2)
    ...
    %PipelineStage (arg0: $DT[$R,$E], arg1: $DT[$R,$E], arg2: $AT[$R,$E]) -> ($AT[$R,$E]) {
    %arg0 = $DT[$R,$E] parameter(0), metadata={op_name="XLA_Args/a"}, backend_config="{}"
    %arg1 = $DT[$R,$E] parameter(1), metadata={op_name="XLA_Args/b"}, backend_config="{}"
    %arg2 = $AT[$R,$E] parameter(2), metadata={op_name="XLA_Args/c"}, backend_config="{}"
    ...
    ROOT %tuple = ($AT[$R,$E]) tuple($AT[$R,$E] %custom-call.2)
    }

    %Pipeline (arg0: $DT[$R,$E], arg1: $DT[$R,$E]) -> $DT[$R,$E] {
    %arg0 = $DT[$R,$E] parameter(0)
    %arg1 = $DT[$R,$E] parameter(1)

    %custom-call.1 = $AT[$R,$E] custom-call($DT[$R,$E] %arg0), custom_call_target="GradientAccumulatorCreate", custom_call_has_side_effect=true, metadata={op_type="GradientAccumulatorCreate" op_name="GradientAccumulatorCreate"}, backend_config="{}"
    %call.1 = ($AT[$R,$E]) call($DT[$R,$E] %arg0, $DT[$R,$E] %arg1, $AT[$R,$E] %custom-call.1), to_apply=%PipelineStage, frontend_attributes={CALL_CONFIG_TYPE=PipelineStageBackward}, metadata={op_type="PipelineStageBackward" op_name="gradients/pipeline_stage_1/PipelineStage_grad/PipelineStageBackward"}, backend_config="{\"callConfig\":{\"type\":\"PipelineStageBackward\",\"pipelineStageConfig\":{\"stageId\":\"1\"}}}"
    %get-tuple-element.1 = $AT[$R,$E] get-tuple-element(($AT[$R,$E]) %call.1), index=0
    ...
    ROOT %call.2 = $DT[$R,$E] call($DT[$R,$E] %arg0, $DT[$R,$E] %arg1, $AT[$R,$E] %custom-call.2), to_apply=%WeightUpdate, frontend_attributes={CALL_CONFIG_TYPE=ResourceUpdate}, metadata={op_type="ResourceUpdate" op_name="ResourceUpdate"}, backend_config="{\"callConfig\":{\"type\":\"ResourceUpdate\",\"resourceUpdateConfig\":{\"numBatchesToAccumulate\":\"$BN\",\"offloadVariables\":\"THREESTATE_ON\"}}}"
    }

    ENTRY %main (arg0: $DT[$R,$E], arg1: $DT[$R,$E]) -> $DT[$R,$E] {
    %arg0 = $DT[$R,$E] parameter(0), parameter_replication={false}, metadata={op_name="XLA_Args/a"}, backend_config="{}"
    %arg1 = $DT[$R,$E] parameter(1), parameter_replication={false}, metadata={op_name="XLA_Args/b"}, backend_config="{}"
    ROOT %call.1 = $DT[$R,$E] call($DT[$R,$E] %arg0, $DT[$R,$E] %arg1), to_apply=%Pipeline, frontend_attributes={CALL_CONFIG_TYPE=Pipeline}, metadata={op_type="Pipeline" op_name="pipeline/Pipeline"}, backend_config="{\"callConfig\":{\"type\":\"Pipeline\",\"pipelineConfig\":{\"gradientAccumulationCount\":\"$BN\",\"repeatCount\":\"2\",\"batchSerializationIterations\":\"$BI\",\"schedule\":\"Interleaved\"}}}"
    }
    )";
}
