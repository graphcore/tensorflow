...
ENTRY cluster {
  ...
  EXPECT_EQ(pipeline_computation->root_instruction(), *stages.resource_update);
  TF_ASSERT_OK_AND_ASSIGN(bool changed, fixer.Run(module.get()));
  EXPECT_TRUE(changed);
  auto root = pipeline_computation->root_instruction();
  EXPECT_NE(root, *stages.resource_update);
  EXPECT_THAT(root->opcode(), HloOpcode::kTuple);
  for (HloInstruction* op : root->operands()) {
    EXPECT_THAT(op->opcode(), HloOpcode::kGetTupleElement);
    EXPECT_EQ(op->operand(0), *stages.resource_update);
  }
}
