...
ENTRY e {
  ...
  EXPECT_TRUE(Match(
      root, m::Tuple(m::Parameter(0), m::Add(m::Op(&recv), m::Parameter(1)),
                     m::Op(&copy))));
  EXPECT_EQ(copy->operand(0), recv);
...
TEST_F(PipelineRecomputationStageInserterTest, TestTokenInFwdStage) {
  std::string hlo = R"(
HloModule top
...
pipeline {
  ...
  in2 = f32[1,4,4,2] parameter(1)
  after-all = token[] after-all()
  stage_0 = (f32[1,4,4,2], f32[1,4,4,2]) call(in1, in2), to_apply=stage_0_fwd, backend_config="{\"callConfig\":{\"type\":\"PipelineStage\",\"pipelineStageConfig\":{\"stageId\":\"0\"}}}", sharding={maximal device=0}
  stage_0_0 = f32[1,4,4,2] get-tuple-element(stage_0), index=0
  stage_0_1 = f32[1,4,4,2] get-tuple-element(stage_0), index=1
  stage_1 = (f32[1,4,4,2], f32[1,4,4,2]) call(stage_0_0, stage_0_1), to_apply=stage_1_fwd, backend_config="{\"callConfig\":{\"type\":\"PipelineStage\",\"pipelineStageConfig\":{\"stageId\":\"1\"}}}", sharding={maximal device=0}
...
TEST_F(PipelineRecomputationStageInserterTest, ExtraneousOpsInFwdStage) {
  std::string hlo = R"(
HloModule top
...
ENTRY e {
  ...
  // Check that there is a copy instruction.
  HloInstruction* root = stages.forward[0]->to_apply()->root_instruction();
  EXPECT_TRUE(Match(root, m::Tuple(m::Parameter(0), m::Parameter(1))));

  // Make sure extraneous instructions have been remove
  HloComputation* recomputation_comp = stages.recomputation.at(0)->to_apply();
  EXPECT_EQ(recomputation_comp->instruction_count(), 3);
}

TEST_F(PipelineRecomputationStageInserterTest, OpaqueParameter) {
  std::string hlo = R"(
HloModule top
...
pipeline {
  ...
  in1 = f32[1,4,4,2] parameter(0)
  in2 = f32[1,4,4,2] parameter(1)
  in3 = opaque[] parameter(2)
  stage_0 = (f32[1,4,4,2], f32[1,4,4,2]) call(in1, in0, in3), to_apply=stage_0_fwd, backend_config="{\"callConfig\":{\"type\":\"PipelineStage\",\"pipelineStageConfig\":{\"stageId\":\"0\"}}}", sharding={maximal device=0}
  stage_0_0 = f32[1,4,4,2] get-tuple-element(stage_0), index=0
  stage_0_1 = f32[1,4,4,2] get-tuple-element(stage_0), index=1
  stage_1 = (f32[1,4,4,2], f32[1,4,4,2]) call(stage_0_0, stage_0_1), to_apply=stage_1_fwd, backend_config="{\"callConfig\":{\"type\":\"PipelineStage\",\"pipelineStageConfig\":{\"stageId\":\"1\"}}}", sharding={maximal device=0}
  stage_1_1 = f32[1,4,4,2] get-tuple-element(stage_1), index=0
...
ENTRY e {
  e.in0 = f32[1,4,4,2] parameter(0), parameter_replication={false}
  e.in1 = f32[1,4,4,2] parameter(1), parameter_replication={false}
  e.in2 = opaque[] parameter(2)
  ROOT e.call = (f32[1,4,4,2], f32[1,4,4,2], opaque[]) call(e.in0, e.in1, e.in2), to_apply=pipeline, backend_config="{\"callConfig\":{\"type\":\"Pipeline\"}}"
}
)";
  auto config = GetModuleConfigForTest();
  TF_ASSERT_OK_AND_ASSIGN(auto module,
  ...
  // Check there wasn't any recomputation.
  EXPECT_TRUE(stages.recomputation.empty());
}
