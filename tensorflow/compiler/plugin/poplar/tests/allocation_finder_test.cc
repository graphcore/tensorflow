...
TEST_F(AllocationFinderTest, FindRecvDoneAllocation) {
  // Check that allocation finder works with tensors from recv-done.
  std::string hlo = R"(
HloModule top

ENTRY %top (arg: f32[1,1,2,2]) -> f32[1,1,2,2] {
  %after-all = token[] after-all()
  %recv = (f32[1,1,2,2]{3,2,1,0}, u32[], token[]) recv(token[] %after-all), channel_id=1, is_host_transfer=true
  %recv-done = (f32[1,1,2,2]{3,2,1,0}, token[]) recv-done((f32[1,1,2,2]{3,2,1,0}, u32[], token[]) %recv), channel_id=1, is_host_transfer=true
  %get-tuple-element = f32[1,1,2,2]{3,2,1,0} get-tuple-element((f32[1,1,2,2]{3,2,1,0}, token[]) %recv-done), index=0
  %arg = f32[1,1,2,2]{3,2,1,0} parameter(0), parameter_replication={false}
  ROOT %convolution = f32[1,1,2,2]{3,2,1,0} convolution(f32[1,1,2,2]{3,2,1,0} %get-tuple-element, f32[1,1,2,2]{3,2,1,0} %arg), window={size=1x1}, dim_labels=b01f_01io->b01f
}
  ...
  const auto* conv_input = convolution->operand(0);
  const auto* recv_done = conv_input->operand(0);

  CompilerAnnotations annotations(module);

  AllocationFinder finder(annotations);
  ASSERT_TRUE(finder.Run(module).ValueOrDie());
...
TEST_F(AllocationFinderTest, FindRecvDoneForwardAllocation) {
  std::string hlo = R"(
HloModule top
...
ENTRY top {
 ...
 c0 = f32[] constant(1)
 after-all = token[] after-all()
 recv = (f32[8], u32[], token[]) recv(token[] after-all), channel_id=1, is_host_transfer=true
 recv-done = (f32[8], token[]) recv-done((f32[8], u32[], token[]) recv), channel_id=1, is_host_transfer=true
 bn_offset = f32[8] get-tuple-element((f32[8], token[]) recv-done), index=0
 p4 = f32[8] parameter(3)
 p5 = f32[8] parameter(4)
 ROOT batch-norm-inf = f32[1,4,4,8] batch-norm-inference(conv, p2_r, bn_offset, p4, p5), epsilon=0.001, feature_index=3
}
  ...
  const auto* bn_offset = bn->operand(2);
  const auto* recv_done = bn_offset->operand(0);
  ...
  auto tensors_with_layout = annotations.tensors_with_layout;
  ASSERT_EQ(tensors_with_layout.size(), 5);
  ASSERT_TRUE(tensors_with_layout.contains(std::make_pair(ip0, 0)));
  ASSERT_TRUE(tensors_with_layout.contains(std::make_pair(ip1, 0)));
  ASSERT_TRUE(tensors_with_layout.contains(std::make_pair(p2_r, 0)));
  ASSERT_TRUE(tensors_with_layout.contains(std::make_pair(ip2, 0)));
  ASSERT_TRUE(tensors_with_layout.contains(std::make_pair(bn_offset, 0)));
}
