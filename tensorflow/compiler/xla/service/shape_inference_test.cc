...
class BatchNormShapeInferenceTest : public ShapeInferenceTest {
  protected:
    // Define test shapes.
    const absl::Span<const int64> dims = {{4, 16, 16, 4}};
    const Shape input_f32 = ShapeUtil::MakeShape(F32, dims);
    const Shape input_f16 = ShapeUtil::MakeShape(F16, dims);
    const Shape four_elem_f32 = ShapeUtil::MakeShape(F32, {dims[3]});
    const Shape four_elem_f16 = ShapeUtil::MakeShape(F16, {dims[3]});
    const int64 feature_index = 3;

    void assert_bad_type(const StatusOr<Shape> &statusor,
                         const std::string &var_name) {
      std::stringstream ss;
      ss << "Expected " << var_name << " to be of type F32 in";

      ASSERT_FALSE(statusor.ok());
      EXPECT_THAT(statusor.status().error_message(), HasSubstr(ss.str()))
      << statusor.status();
    }
};

TEST_F(BatchNormShapeInferenceTest, BatchNormTraining) {
  TF_ASSERT_OK_AND_ASSIGN(StatusOr<Shape> bn_training_shape_f32,
                          ShapeInference::InferBatchNormTrainingShape(
                              input_f32, four_elem_f32, four_elem_f32,
                              feature_index));

  TF_ASSERT_OK_AND_ASSIGN(StatusOr<Shape> bn_training_shape_f16,
                          ShapeInference::InferBatchNormTrainingShape(
                              input_f16, four_elem_f32, four_elem_f32,
                              feature_index));
}

TEST_F(BatchNormShapeInferenceTest, BatchNormTrainingBadScale) {
  StatusOr<Shape> statusor = ShapeInference::InferBatchNormTrainingShape(
      input_f16, four_elem_f16, four_elem_f32, feature_index);

  assert_bad_type(statusor, "scale");
}

TEST_F(BatchNormShapeInferenceTest, BatchNormTrainingBadOffset) {
  StatusOr<Shape> statusor = ShapeInference::InferBatchNormTrainingShape(
      input_f16, four_elem_f32, four_elem_f16, feature_index);

  assert_bad_type(statusor, "offset");
}

TEST_F(BatchNormShapeInferenceTest, BatchNormInference) {
  TF_ASSERT_OK_AND_ASSIGN(StatusOr<Shape> bn_inference_shape_f32,
                          ShapeInference::InferBatchNormInferenceShape(
                              input_f32, four_elem_f32, four_elem_f32,
                              four_elem_f32, four_elem_f32, feature_index));

  TF_ASSERT_OK_AND_ASSIGN(StatusOr<Shape> bn_inference_shape_f16,
                          ShapeInference::InferBatchNormInferenceShape(
                              input_f16, four_elem_f32, four_elem_f32,
                              four_elem_f32, four_elem_f32, feature_index));
}

TEST_F(BatchNormShapeInferenceTest, BatchNormInferenceBadScale) {
  StatusOr<Shape> statusor = ShapeInference::InferBatchNormInferenceShape(
      input_f16, four_elem_f16, four_elem_f32, four_elem_f32, four_elem_f32, 
      feature_index);

  assert_bad_type(statusor, "scale");
}

TEST_F(BatchNormShapeInferenceTest, BatchNormInferenceBadOffset) {
  StatusOr<Shape> statusor = ShapeInference::InferBatchNormInferenceShape(
      input_f16, four_elem_f32, four_elem_f16, four_elem_f32, four_elem_f32, 
      feature_index);

  assert_bad_type(statusor, "offset");
}

TEST_F(BatchNormShapeInferenceTest, BatchNormInferenceBadMean) {
  StatusOr<Shape> statusor = ShapeInference::InferBatchNormInferenceShape(
      input_f16, four_elem_f32, four_elem_f32, four_elem_f16, four_elem_f32, 
      feature_index);

  assert_bad_type(statusor, "mean");
}

TEST_F(BatchNormShapeInferenceTest, BatchNormInferenceBadVariance) {
  StatusOr<Shape> statusor = ShapeInference::InferBatchNormInferenceShape(
      input_f16, four_elem_f32, four_elem_f32, four_elem_f32, four_elem_f16, 
      feature_index);

  assert_bad_type(statusor, "variance");
}

TEST_F(BatchNormShapeInferenceTest, BatchNormGradBadScale) {
  StatusOr<Shape> statusor = ShapeInference::InferBatchNormGradShape(
      input_f16, four_elem_f16, four_elem_f32, four_elem_f32, four_elem_f32, 
      feature_index);
...
TEST_F(BatchNormShapeInferenceTest, BatchNormGradBadMean) {
  StatusOr<Shape> statusor = ShapeInference::InferBatchNormGradShape(
      input_f16, four_elem_f32, four_elem_f16, four_elem_f32, four_elem_f32, 
      feature_index);
...
TEST_F(BatchNormShapeInferenceTest, BatchNormGradBadVariance) {
  StatusOr<Shape> statusor = ShapeInference::InferBatchNormGradShape(
      input_f16, four_elem_f32, four_elem_f32, four_elem_f16, four_elem_f32, 
      feature_index);
...
TEST_F(BatchNormShapeInferenceTest, BatchNormGradBadGradOutput) {
  StatusOr<Shape> statusor = ShapeInference::InferBatchNormGradShape(
      input_f16, four_elem_f32, four_elem_f32, four_elem_f32, four_elem_f32, 
      feature_index);

  ASSERT_FALSE(statusor.ok());
  EXPECT_THAT(statusor.status().error_message(), 
              HasSubstr("Expected output_grad to be of type F16"))
  << statusor.status();
}
